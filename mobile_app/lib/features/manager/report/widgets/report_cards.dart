...
class NewReportCard extends StatefulWidget {
  const NewReportCard({required this.roster, required this.onSubmit, required this.onCancel, super.key});

  final List<AllowlistEntry> roster;
  final Future<void> Function({required String email, required String name, required DateTime date, required String reasonHtml}) onSubmit;
...
class _NewReportCardState extends State<NewReportCard> {
  ...
  bool get _canSubmit => _email != null && !_reasonController.isBlank && !_isSubmitting;

  Future<void> _submit() async {
    final entry = widget.roster.firstWhere((u) => u.email == _email);
    setState(() => _isSubmitting = true);
    try {
      await widget.onSubmit(email: entry.email, name: entry.name, date: _date, reasonHtml: _reasonController.html);
    } finally {
        ...
        child: Column(
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              ...
              items: [for (final u in widget.roster) DropdownMenuItem(value: u.email, child: Text(u.name))],
            ...
            Align(
              ...
              child: TextButton(onPressed: () => setState(() => _date = DateTime.now()), child: const Text('Today')),
            ),
            const SizedBox(height: 8),
            RichTextEditor(controller: _reasonController, placeholder: 'Why is this being flagged?', minLines: 3),
            const SizedBox(height: 12),
            Row(
              children: [
                Expanded(
                  child: OutlinedButton(onPressed: _isSubmitting ? null : widget.onCancel, child: const Text('Cancel')),
                ),
...
class OpenReportCard extends StatefulWidget {
  const OpenReportCard({required this.report, required this.onResolve, super.key});
          ...
          children: [
            Row(
              children: [
                ...
                Expanded(
                  child: Text(r.name, style: const TextStyle(fontWeight: FontWeight.w700)),
                ),
                if (r.date != null) Text(_dateFmt.format(r.date!), style: TextStyle(color: AppColors.ink500)),
              ],
            ),
            const SizedBox(height: 8),
            if (r.reasonHtml.isNotEmpty) HtmlWidget(r.reasonHtml) else const Text('No reason provided.'),
            Text(
            ...
            if (r.rejectionNote.isNotEmpty) ...[
              ...
              Container(
                ...
                decoration: BoxDecoration(color: AppColors.statusRejectedBg, borderRadius: BorderRadius.circular(8)),
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('SENT BACK BY YOU', style: TextStyle(color: AppColors.statusRejected, fontSize: 10, fontWeight: FontWeight.w700)),
                    const SizedBox(height: 4),
            ...
            if (!_resolving)
              Align(
                alignment: Alignment.centerRight,
                child: OutlinedButton(onPressed: () => setState(() => _resolving = true), child: const Text('Resolve')),
              )
            else ...[
              RichTextEditor(controller: _controller, placeholder: 'Explain what happened...', minLines: 3),
              const SizedBox(height: 8),
              Row(
                mainAxisAlignment: MainAxisAlignment.end,
                children: [
                  TextButton(
                    onPressed: _isSubmitting ? null : () => setState(() => _resolving = false),
...
class ExplainedReportCard extends StatefulWidget {
  const ExplainedReportCard({required this.report, required this.onAccept, required this.onReject, super.key});
...
class _ExplainedReportCardState extends State<ExplainedReportCard> {
  ...
  Future<void> _submit() async {
    if (_deciding == false && _controller.isBlank) return; // reject requires a note
    setState(() => _isSubmitting = true);
    try {
      if (_deciding!) {
          ...
          children: [
            Row(
              children: [
                ...
                if (r.explainedAt != null)
                  Text('Explained ${_timeFmt.format(r.explainedAt!)}', style: TextStyle(color: AppColors.ink500, fontSize: 11)),
              ],
            ),
            const SizedBox(height: 8),
            Text('REPORTED REASON', style: TextStyle(color: AppColors.ink500, fontSize: 10, fontWeight: FontWeight.w700)),
            HtmlWidget(r.reasonHtml.isNotEmpty ? r.reasonHtml : '<i>No reason provided.</i>'),
            const SizedBox(height: 8),
            Text("DEVELOPER'S EXPLANATION", style: TextStyle(color: AppColors.ink500, fontSize: 10, fontWeight: FontWeight.w700)),
            ...
            if (_deciding == null)
                ...
                children: [
                  ...
                  Expanded(
                    child: ElevatedButton(
                      ...
                      style: ElevatedButton.styleFrom(backgroundColor: AppColors.statusApproved, foregroundColor: Colors.white),
              ...
              RichTextEditor(
                ...
                placeholder: _deciding! ? 'Note (optional)' : 'Reason for sending this back',
                minLines: 2,
              ),
              const SizedBox(height: 8),
              Row(
                ...
                children: [
                  TextButton(
                    onPressed: _isSubmitting ? null : () => setState(() => _deciding = null),
                    child: const Text('Cancel'),
                  ),
                  const SizedBox(width: 8),
                  ElevatedButton(
                    onPressed: _isSubmitting || (_deciding == false && _controller.isBlank) ? null : _submit,
                    style: ElevatedButton.styleFrom(
                      backgroundColor: _deciding! ? AppColors.statusApproved : AppColors.statusRejected,
          ...
          children: [
            Row(
              children: [
                ...
                const SizedBox(width: 10),
                Expanded(child: Text(report.name, style: const TextStyle(fontWeight: FontWeight.w700))),
                if (report.resolvedAt != null)
                  Text(_timeFmt.format(report.resolvedAt!), style: TextStyle(color: AppColors.ink500, fontSize: 11)),
              ],
            ),
            ...
            if (report.resolvedBy.isNotEmpty)
              Text('Resolved by ${report.resolvedBy}', style: TextStyle(color: AppColors.ink500, fontSize: 12)),
          ],
        ),
      ),
    );
